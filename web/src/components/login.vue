...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      console.log(_this.MODAL_STATUS);
    },
    methods: {
      /**
       * 打开登录注册窗口
      ...
      openLoginModal() {
        let _this = this;
        if(_this.MODAL_STATUS===_this.STATUS_LOGIN){
          _this.toLoginDiv();
        }
      ...
      sendSmsForRegister() {
        ...
        _this.$ajax.get('http://127.0.0.1:9000/business/web/member/is-mobile-exist/' + _this.memberRegister.mobile).then((res)=>{
      ...
      sendSmsCode(sms, btnId) {
        ...
        // 调服务端发短信接口
        _this.$ajax.post('http://127.0.0.1:9000/business/web/sms/send', sms).then((res)=> {
          let response = res.data;
          if (response.success) {
            Toast.success("短信已发送");
      ...
      sendSmsForForget() {
        ...
        _this.$ajax.get('http://127.0.0.1:9000/business/web/member/is-mobile-exist/' + _this.memberForget.mobile).then((res)=>{
          let response = res.data;
          if (response.success) {
